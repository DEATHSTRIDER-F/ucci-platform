...
interface EditGalleryFormClientProps {
  post: {
    ...
    title: string
    content: string | null
    post_type?: GalleryPostType | null
...
export function EditGalleryFormClient({ post, areas, adminProfile }: EditGalleryFormClientProps) {
  ...
  const [postType, setPostType] = useState<GalleryPostType>(post.post_type ?? 'event')
    ...
    <div className="glass-card p-8 space-y-6 max-w-3xl">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          <label className="block text-brand-silver text-sm font-medium">Images {postType === 'event' ? '*' : <span className="text-brand-silver/50">(optional for news with YouTube link)</span>}</label>
