...
export function LoginForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <div>
        <label htmlFor="login_email" className="block text-brand-silver text-sm font-medium mb-1">Email Address</label>
        <input
          id="login_email"
          type="email"
          value={email}
      ...
      <div>
        <label htmlFor="login_password" className="block text-brand-silver text-sm font-medium mb-1">Password</label>
      ...
      <button type="submit" disabled={pending} className="btn-primary w-full flex items-center justify-center gap-2 text-base">
        {pending ? <><Loader2 className="w-5 h-5 animate-spin" /> Signing in...</> : 'Sign In'}
      </button>
    </form>
  )
}
