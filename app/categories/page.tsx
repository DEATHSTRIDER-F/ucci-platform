          ...
          {categoriesWithCounts.map(cat => {
            ...
            return (
              <Link
                ...
                className="glass-card p-6 group hover:border-brand-gold/50 hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col"
              >
                <div className="flex items-center gap-3 mb-1">
                  <CategoryIcon name={iconName} color={iconColor} size={28} />
                  <h2 className="font-display font-semibold text-brand-white group-hover:text-brand-gold transition-colors truncate flex-1 min-w-0">
                    {cat.name}
                  </h2>
                </div>
                {cat.meta_description && (
                  <p className="text-brand-silver text-sm line-clamp-2 mt-1 flex-1">{cat.meta_description}</p>
                )}
                <div className="mt-3 flex items-center justify-between gap-3">
                  ...
                  <span className="flex items-center gap-2 shrink-0">
                    ...
                    <ArrowRight className="w-5 h-5 text-brand-gold/40 group-hover:text-brand-gold group-hover:translate-x-1 transition-all" />
                  </span>
                </div>
              </Link>
            )
